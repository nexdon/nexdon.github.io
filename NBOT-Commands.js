...
window.botCommand = {
    cat: function (id) {
        ...
        if (!args.trim()) {
            ...
            send(':' + id + ' ' + args.replace(/\[random\]/g, function() { return arr[Math.floor(Math.random() * arr.length)] }).replace(/\[ping random\]/g, function() { return '@' + arrp[Math.floor(Math.random() * arrp.length)] }));
        }
    }, online: function (id) {
        var bots = [
            'NBOT',
